...
exports.uploadFotoProfil = async (req, res, next)=>{  
  var user = new User();
  user.findByIdAndUpdate(req.params.id, function(error, userUpdate){        
    userUpdate.picture.data = fs.readFileSync(req.files.userUpdatePhoto.path);
    userUpdate.picture.contentType = 'image/png';
    userUpdate.save();
  }); 
  user.findById(req.params.id, function (err, doc) {
    if (err) {console.log(err)};
    var base64 = doc.img.data.toString('base64');
    res.json({'data':doc.img.contentType,'base64':base64});   
  });
}
...
exports.uploadImageProfil = async (req, res, next)=>{            
  ...
  User.findByIdAndUpdate({'_id':id},{'link' : linknya }, (err, result) => {      
    return result;
  });
  const fil = [req.file];
  insertDocuments(req.params.id, fil,(file)=>{
